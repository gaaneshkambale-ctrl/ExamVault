...
public class FakeTenantRepository : ITenantRepository
{
    ...
    public Task<Tenant?> GetByNameAsync(string name, CancellationToken cancellationToken = default) =>
        Task.FromResult(_tenants.FirstOrDefault(t => t.Name == name));
    ...
    public Task DeleteTenantScopedDataAsync(Guid tenantId, CancellationToken cancellationToken = default) =>
        Task.CompletedTask;

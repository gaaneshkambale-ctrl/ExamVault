...
// Percentage change vs the prior month. Returns null when there's no
// meaningful comparison (both periods empty), so the caller can hide the
// badge instead of showing a misleading "+100%" or divide-by-zero artifact.
...
export default function InstructorDashboard() {
  ...
  const { attemptsByExam, isLoading: isLoadingAttempts } = useAttemptsByExam(publishedExamIds);
  ...
  // Sorted newest-first, with real pagination rather than a fixed top-N slice.
  ...
  // Same "sum assignment targetCount per exam" pattern Active Exams uses to
  // answer "how many students are expected to take this exam".
    ...
    <InstructorLayout active="Dashboard">
      <div className="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-4">
        <div>
          ...
          <p className="text-muted mb-0">Here's an overview of the exams you've authored.</p>
        </div>
          ...
          <Card className="border-0 shadow-sm h-100">
            <Card.Body>
              ...
              {isLoadingResults ? (
                ...
                <DualTrendChart data={dailyActivity} seriesALabel="Exams Conducted" seriesBLabel="Students Participated" />
              )}
        ...
        <Col xs={12} lg={4}>
          <Card className="border-0 shadow-sm h-100">
            <Card.Body>
              ...
              <QuickActionRow to="/admin/live-monitoring/active-exams" label="Live Monitoring" icon={<ActiveExamsIcon />} />
              <QuickActionRow to="/admin/results/exams" label="View Results" icon={<ResultsSmallIcon />} />
              <QuickActionRow to="/admin/reports/exams" label="View Reports" icon={<ReportsIcon />} />
              <QuickActionRow to="/admin/notifications/create" label="Send Notification" icon={<BellSmallIcon />} />
            </Card.Body>
          </Card>
        </Col>
      </Row>
        ...
        <Col xs={12} lg={5}>
          <Card className="border-0 shadow-sm h-100">
            <Card.Body className={upcomingAssignments.length === 0 ? '' : 'p-0'}>
              <div className="p-4 pb-0">
                <SectionHeader icon={<ClockIcon />} title="Upcoming Exams" />
              </div>
                  ...
                  <tbody>
                    {upcomingAssignments.map((a) => (
                      <tr key={a.id}>
                        ...
                        <td>{new Date(a.startAtUtc).toLocaleDateString()}</td>
                        <td className="pe-4">
